...
public class LeaveDeptLeaderAssignProcessor extends OaBaseTaskListenerService implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		...
		List<AtiUser> users = (List<AtiUser>) runtimeService.getVariable(delegateTask.getProcessInstanceId(), processDefinition.getKey() + ProcDefKey.LEAVE_TASK_DEF_KEY[2]);
		...
		//正常情况下节点设置候选人
		//部门经理角色编号
		AtiRole atiRole = atiRoleDao.getRoleByRoleCode("2000");
		...
		taskService.setVariable(delegateTask.getId(),ProcDefKey.LEAVE_TASK_DEF_KEY[2],users);
